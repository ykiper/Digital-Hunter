from validations import Damage
...
def process_attack(data):
    ...
    try:
        validate_data = Damage.model_validate(data)
        data_dict = validate_data.model_dump()
        query = {"entity_id": data_dict["entity_id"]}

        doc = mongo.collection.find(query)
        if doc:
            mongo.collection.update_one({"entity_id": data_dict["entity_id"]}, {"$set":{"attack_result": doc["result"]}})
